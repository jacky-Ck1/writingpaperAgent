...
class RAGAgent(BaseAgent):
    ...
    def __init__(self, llm=None, vectorstore=None, retrieval_k=None, hybrid_retriever=None):
        """
        初始化 RAG Agent
        ...
        Args:
            ...
            hybrid_retriever: HybridRetriever 实例（可选，传入后启用 BM25+Dense+RRF+Reranker 混合检索）
        """
        # 初始化 LLM
    ...
    def retrieve(self, query: str) -> list:
        ...
        # 优先使用混合检索 (BM25 + Dense + RRF + Reranker)
        if self.hybrid_retriever is not None:
            print(f"[RAGAgent] 使用混合检索 (BM25 + Dense + RRF + Reranker)")
            results = self.hybrid_retriever.retrieve(
                query, top_k=self.retrieval_k,
                use_reranker=Config.ENABLE_HYBRID_RETRIEVAL,
            )
            docs = [doc for doc, _score in results]
            print(f"[RAGAgent] 检索到 {len(docs)} 个相关文档片段 (混合)")
            return docs

        # 回退：使用 MMR (最大边际相关性) 检索，避免结果过于相似
